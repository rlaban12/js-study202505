...
// 회원 목록에서 취미가 딱 2개인 사람들만 추려서
// 새 배열에 다시 담아주세요.

/*
// 내가 푼 것
const search = function (userList) {
  let obj = {};
  for (let i = 0; i < userList.length; i++) {
    if (userList['hobbies'].length === 2) {
      obj = userList[i];
    }
  }
  return obj;
...
let resultObj = search(userList);
console.log(resultObj);*/

// 선생님 해설-----------------------------------------------------------------------
// 회원 목록에서 취미가 딱 N개인 사람들만 추려서
...
const filter6 = filter(names, names => names.includes('김'));
console.log(filter6);
